...
%select the first 5 rows for each subject RECEIVE AVOID martetup/down if datanew(k,5)>0
    ...
    sumExp=0;
       ...
       if datanew(k,5)>0
            ...
            if datanew(k,6)==NaN
                ...
                sumExp=sumExp+datanew(k,6);
            end
       end
     binsMatrix(k,1)= binsMatrix(k,1) + up;
     binsMatrix(k,2)= binsMatrix(k,2) + sumExp;
